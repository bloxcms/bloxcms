    ...
    if (isset($_GET['login'])) {
        ...
        if ($usersInfo)
        {
            ...
            {   
                ...
                if ($_GET['password-update-href']) { # Use a custom template
                    $h = Url::decode($_GET['password-update-href']);
            ...
            if ($messageIsNotSent)  {
                ...
                $mdat2 = $terms['error-message5'].' '.dirname(Url::punyDecode($_SERVER['HTTP_HOST']).$_SERVER['REQUEST_URI']."x").'. '.$messageIsNotSent;
                if (empty($fromUserInfo['email'])) 
